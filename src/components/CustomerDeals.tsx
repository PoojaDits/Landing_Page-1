...
const CustomerDeals: React.FC = () => {
  ...
  const fetchDeals = async () => {
    ...
    try {
      ...
      // Filter products that have originalPrice (i.e. on sale)
      const onSale = data.filter(p => p.originalPrice && p.originalPrice > (p.price || 0))
      setDeals(onSale.length > 0 ? onSale : data.slice(0, 8)) // fallback to first 8 if no discounts
    } catch {
      // Fallback demo deals
      setDeals([
        {
    ...
    <div className="max-w-7xl mx-auto p-6 sm:p-8">
      ...
      <div className="mb-8">
        ...
        <p className="text-slate-300 mt-2 max-w-2xl">
          Grab these amazing discounts before they're gone. Prices won't stay this low for long!
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-20">
          ...
          <p className="text-xl text-white">No active deals right now</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {deals.map((deal) => {
            ...
            return (
              <div key={deal.id} className="group bg-slate-950/90 rounded-3xl overflow-hidden border border-white/10 flex flex-col transition-all hover:border-white/30 hover:shadow-xl hover:shadow-black/20">
                {/* Image */}
                ...
                <div className="p-5 flex flex-col flex-1">
                  ...
                  <div className="flex gap-3 mt-5">
                    <Link
                      to={`/customer/product/${deal.id}`}
                      className="flex-1 text-center border border-white/20 hover:bg-white/5 text-white text-sm font-medium py-2.5 rounded-2xl transition"
                    >
                      View Details
                    </Link>
                    <button
                      onClick={() => handleAddToCart(deal)}
                      className="flex-1 bg-[#e94560] hover:bg-[#c73652] text-white text-sm font-semibold py-2.5 rounded-2xl transition"
                    >
                      Add to Cart
                    </button>
                  </div>
      ...
      <div className="mt-10 text-center text-sm text-gray-400">
        Deals are updated daily. All prices include taxes.
      </div>
    </div>
  )
}
